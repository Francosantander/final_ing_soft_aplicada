...
describe('Book e2e test', () => {
  ...
  const bookSample = { isbn: 'merchandise t', name: 'redundant', publishYear: 'solutions', copies: 30433 };
  ...
  describe('new Book page', () => {
    ...
    it('should create an instance of Book', () => {
      cy.get(`[data-cy="isbn"]`).type('male as');
      cy.get(`[data-cy="isbn"]`).should('have.value', 'male as');

      cy.get(`[data-cy="name"]`).type('1080p Van alliance');
      cy.get(`[data-cy="name"]`).should('have.value', '1080p Van alliance');

      cy.get(`[data-cy="publishYear"]`).type('Bronze Shoes Blues');
      cy.get(`[data-cy="publishYear"]`).should('have.value', 'Bronze Shoes Blues');

      cy.get(`[data-cy="copies"]`).type('23886');
      cy.get(`[data-cy="copies"]`).should('have.value', '23886');
